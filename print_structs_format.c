...
void print_instruccion(Instruction *my_instruccion) {
    ...
    printf_color("Mod_rm:       %02X = ", *((uint8_t*)&(my_instruccion->Mod_rm)));
    print_binary( *((uint8_t*)&(my_instruccion->Mod_rm)), 8);
    printf_color("mod:          %02X = ", my_instruccion->Mod_rm.mod); print_binary(my_instruccion->Mod_rm.mod, 2);
    printf_color("reg:          %02X =   ", my_instruccion->Mod_rm.reg); print_binary(my_instruccion->Mod_rm.reg, 3);
    printf_color("R_M:          %02X =       ", my_instruccion->Mod_rm.R_M); print_binary(my_instruccion->Mod_rm.R_M, 3);
    ...
    if ((*((uint8_t*)&(my_instruccion->Mod_rm)) & 0b11000111) == 0b00000100){ // modo SIB
        ...
        printf_color("scale:        %02X = ", my_instruccion->SIB.scale); print_binary(my_instruccion->SIB.scale, 2);
        printf_color("index:        %02X =   ", my_instruccion->SIB.index); print_binary(my_instruccion->SIB.index, 3);
        printf_color("base:         %02X =       ",  my_instruccion->SIB.base); print_binary(my_instruccion->SIB.base, 3);
