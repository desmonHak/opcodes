...
void print_opcode(opcode opcode, uint8_t number_opcode) {
    ...
    print_binary(opcode.opcode_bits_final.opcode<< 2, 8);
...
void print_instruccion(Instruction *my_instruccion) {
    ...
    printf_color("mod:          #{FG:lgreen}%02X#{FG:reset} = ", my_instruccion->Mod_rm.mod); print_binary(my_instruccion->Mod_rm.mod, 2);
    printf_color("reg:          #{FG:lgreen}%02X#{FG:reset} =   ", my_instruccion->Mod_rm.reg); print_binary(my_instruccion->Mod_rm.reg, 3);
    printf_color("R_M:          #{FG:lgreen}%02X#{FG:reset} =       ", my_instruccion->Mod_rm.R_M); print_binary(my_instruccion->Mod_rm.R_M, 3);
    ...
    if ((*((uint8_t*)&(my_instruccion->Mod_rm)) & 0b11000111) == 0b00000100){ // modo SIB
        ...
        printf_color("scale:        #{FG:lgreen}%02X#{FG:reset} = ", my_instruccion->SIB.scale); print_binary(my_instruccion->SIB.scale, 2);
        printf_color("index:        #{FG:lgreen}%02X#{FG:reset} =   ", my_instruccion->SIB.index); print_binary(my_instruccion->SIB.index, 3);
        printf_color("base:         #{FG:lgreen}%02X#{FG:reset} =       ",  my_instruccion->SIB.base); print_binary(my_instruccion->SIB.base, 3);
